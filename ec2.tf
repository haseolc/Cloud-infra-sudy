...
resource "aws_instance" "AWS_ec2_sg_7th_room" {
  ami           = data.aws_ami.amazon_linux_2.id         # 아마존 리눅스 2 최신 AMI
  ...
  vpc_security_group_ids = [aws_security_group.sg_7th_room.id, aws_security_group.web-sg]   # 보안 그룹 적용
  key_name      = "project1-key"                              # 키페어 이름
  #infra-dev-key
  tags = {
